...
class ShotEditorPage extends StatefulWidget {
  @override
  ShotEditorPageState createState() => ShotEditorPageState();
}

class ShotEditorPageState extends State<ShotEditorPage> {
  final GlobalKey<ShotsDataTableState> shotsDataTableKey =
          ...
          if (state is ShotFetched) {
            return Row(
                ...
                children: [
                  Expanded(
                    flex: 3,
                    child: ShotsDataTable(key: shotsDataTableKey),
                  ),
                  Expanded(
                    flex: 1,
                    child: ShotInspector(key: shotInspectorKey),
                  )
                ]);
          } else {
...
class ShotsDataTable extends StatefulWidget {
  ShotsDataTable({Key key}) : super(key: key);
  @override
  ShotsDataTableState createState() => ShotsDataTableState();
}
...
class ShotsDataTableState extends State<ShotsDataTable> {
  ...
  void _sort(int index, bool ascending) {
    if (ascending) {
      shots.sort((a, b) => a.startTime.compareTo(b.startTime));
    } else {
      shots.sort((a, b) => b.startTime.compareTo(a.startTime));
    }
    setState(() {
                  ...
                  slivers: [
                    SliverList(
                      ...
                      DataTable(
                        ...
                        rows: shots
                            .map((shot) => DataRow(
                                    selected: shotsSelected.contains(shot),
                                    onSelectChanged: (isSelected) {
                                      setState(() {
                                        if (isSelected) {
                                          shotsSelected.add(shot);
                                        } else {
                                          shotsSelected.remove(shot);
                                        }
                                      });
                                    },
                                    cells: [
...
class ShotInspector extends StatefulWidget {
  ShotInspector({Key key}) : super(key: key);
  @override
  ShotInspectorState createState() => ShotInspectorState();
}
  ...
  Widget build(BuildContext context) {
    return Container(
        ...
        child: Column(children: <Widget>[
                                  ...
                                  if (snapshot.hasData) {
                                    return SliderTheme(
                                        ...
                                        child: Slider(
                                          ...
                                          onChangeEnd: (value) => shotBloc
                                              .currentShotTimeSubject
                                              .add([
                                            18100,
                                            22900,
                                            24200,
          ...
          StreamBuilder(
              stream: shotBloc.statisticsStream,
              builder: (BuildContext context, AsyncSnapshot snapshot) {
                if (snapshot.hasData) {
                  return Column(
                      mainAxisAlignment: MainAxisAlignment.start,
          ...
          StreamBuilder(
              stream: shotBloc.coverageStream,
              builder: (BuildContext context, AsyncSnapshot snapshot) {
                if (snapshot.hasData) {
          ...
          //                     text: snapshot.data[i]['lyricContent'],
          //                     style: textStyle,
          //                   );
          //                 })),
          //                 showCursor: true,
